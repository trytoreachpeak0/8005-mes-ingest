...
param(
    ...
    [string] $ResultsDirectory = (Join-Path $PSScriptRoot '.artifacts\ticket15-tests'),
...
if ($connectionString.IndexOf('(localdb)', [StringComparison]::OrdinalIgnoreCase) -ge 0) {
    throw 'Ticket 15 requires a real SQL Server; LocalDB cannot satisfy this gate.'
}
...
$trxName = 'ticket15-oracle-round-source-sqlserver.trx'
...
try {
    ...
    & dotnet test $testProject `
        ...
        --filter 'FullyQualifiedName~OracleMesTaskUnionProductionEntryTests|FullyQualifiedName~Ticket15RoundEvidenceTests' `
        --results-directory $resultsPath `
        --logger "trx;LogFileName=$trxName"
    if ($LASTEXITCODE -ne 0) {
        throw "Ticket 15 SQL Server/API tests failed with exit code $LASTEXITCODE."
    }
    ...
    if ($total -ne $expectedTestCount -or $executed -ne $expectedTestCount -or $passed -ne $expectedTestCount -or $failed -ne 0 -or $notExecuted -ne 0) {
        throw "Ticket 15 gate requires exactly $expectedTestCount passed and 0 skipped; observed total=$total executed=$executed passed=$passed failed=$failed notExecuted=$notExecuted."
    }

    Write-Output "MESINGEST_TICKET15_SQLSERVER_API_GATE_PASSED: passed=$passed skipped=$notExecuted expectedProductMajor=$ExpectedProductMajor expectedCompatibilityLevel=$ExpectedCompatibilityLevel trx=$trxPath"
}
finally {
