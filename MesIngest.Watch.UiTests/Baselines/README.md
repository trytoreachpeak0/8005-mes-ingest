...
.\Test-WatchXamlBaselineStability.ps1 -Runs 3
```
...
The stability script leaves the final `SelectedUi` received candidates in place only after
`-Runs` byte-identical runs (default 3; see "Repetition count" in
`docs/agents/golden-renderer.md` for when to raise it). XAML output is textual, so this
gate stays byte-exact — the visual-equivalence predicate applies to window pixels only. It never renames or accepts them. A human baseline update must include
