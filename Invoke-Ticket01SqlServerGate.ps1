...
param(
    ...
    [string] $ResultsDirectory = (Join-Path $PSScriptRoot '.artifacts\ticket01-tests'),
...
if ($connectionString.IndexOf('(localdb)', [StringComparison]::OrdinalIgnoreCase) -ge 0) {
    throw 'Ticket 01 requires a real SQL Server; LocalDB cannot satisfy this gate.'
}
...
$trxName = 'ticket01-tracer-sqlserver.trx'
...
try {
    ...
    & dotnet test $testProject `
        ...
        --filter 'FullyQualifiedName~NewSuccessRoundTracerSpineTests' `
        --results-directory $resultsPath `
        --logger "trx;LogFileName=$trxName"
    if ($LASTEXITCODE -ne 0) {
        throw "Ticket 01 SQL Server tracer tests failed with exit code $LASTEXITCODE."
    }
    ...
    if ($total -ne 3 -or $executed -ne 3 -or $passed -ne 3 -or $failed -ne 0 -or $notExecuted -ne 0) {
        throw "Ticket 01 gate requires exactly 3 passed and 0 skipped; observed total=$total executed=$executed passed=$passed failed=$failed notExecuted=$notExecuted."
    }

    Write-Output "MESINGEST_TICKET01_SQLSERVER_GATE_PASSED: passed=$passed skipped=$notExecuted expectedProductMajor=$ExpectedProductMajor expectedCompatibilityLevel=$ExpectedCompatibilityLevel trx=$trxPath"
}
finally {
    [Environment]::SetEnvironmentVariable(
        $productMajorVariable,
