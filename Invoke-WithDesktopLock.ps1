...
.PARAMETER TimeoutSeconds
    ...
    `WATCH_UI_SERIALIZATION_BUSY` 契约一致。默认值是给**手动调用**的：人在客户机上跑桌面测试时撞上
    别人，立刻报错比默默等着好。

    **CI 一律传真的超时（workflow 里是 1800）。**2026-09-19 起第二个仓库加进来了：
    `8005-agv-control-server` 的真装置 L2 在同一个交互式桌面上起 WPF 窗口，取同一个名字的互斥体。
    CI 作业不该因为撞上另一个仓的调度就变红，所以它排队。1800 秒与服务端 `scripts/DesktopLock.psm1`
    的默认等待一致；服务端每个真装置场景各拿一次锁、跑完即放，单个场景实测最长约 7 分钟，所以这里等的
    是「对方当前那一个场景」，不是对方整个作业。
...
try {
    try {
        ...
        #
        # 用 `-TimeoutSeconds 0` 走不到这条路：没有人在等，对象就直接不存在了，`WaitOne` 返回 `$true`。
        # CI 传了真超时之后（2026-09-19 起），持有者被杀时正好有人在等，这条路就是真的了。**这四行
        # 仍没有测试覆盖**，要覆盖它得同时起一个持有者和一个阻塞的等待者再杀掉前者。
