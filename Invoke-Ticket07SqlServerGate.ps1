...
param(
    ...
    [string] $ResultsDirectory = (Join-Path $PSScriptRoot '.artifacts\ticket07-tests'),
...
if ($connectionString.IndexOf('(localdb)', [StringComparison]::OrdinalIgnoreCase) -ge 0) {
    throw 'Ticket 07 requires a real SQL Server; LocalDB cannot satisfy this gate.'
}
...
$trxName = 'ticket07-task-type-protection-sqlserver.trx'
...
try {
    ...
    & dotnet test $testProject `
        ...
        --filter 'FullyQualifiedName~TaskTypeProtectionTests' `
        --results-directory $resultsPath `
        --logger "trx;LogFileName=$trxName"
    if ($LASTEXITCODE -ne 0) {
        throw "Ticket 07 SQL Server/API tests failed with exit code $LASTEXITCODE."
    }
    ...
    if ($total -ne 6 -or $executed -ne 6 -or $passed -ne 6 -or $failed -ne 0 -or $notExecuted -ne 0) {
        throw "Ticket 07 gate requires exactly 6 passed and 0 skipped; observed total=$total executed=$executed passed=$passed failed=$failed notExecuted=$notExecuted."
    }

    Write-Output "MESINGEST_TICKET07_SQLSERVER_API_GATE_PASSED: passed=$passed skipped=$notExecuted expectedProductMajor=$ExpectedProductMajor expectedCompatibilityLevel=$ExpectedCompatibilityLevel trx=$trxPath"
}
finally {
    [Environment]::SetEnvironmentVariable(
        $productMajorVariable,
