...
param(
    ...
    [string] $ResultsDirectory = (Join-Path $PSScriptRoot '.artifacts\ticket09-tests'),
...
if ($connectionString.IndexOf('(localdb)', [StringComparison]::OrdinalIgnoreCase) -ge 0) {
    throw 'Ticket 09 requires a real SQL Server; LocalDB cannot satisfy this gate.'
}
...
$trxName = 'ticket09-externally-readable-catalog-sqlserver.trx'
...
try {
    ...
    & dotnet test $testProject `
        ...
        --filter 'FullyQualifiedName~ExternallyReadableDemandCatalogTests' `
        --results-directory $resultsPath `
        --logger "trx;LogFileName=$trxName"
    if ($LASTEXITCODE -ne 0) {
        throw "Ticket 09 SQL Server/API tests failed with exit code $LASTEXITCODE."
    }
    ...
    if ($total -ne 15 -or $executed -ne 15 -or $passed -ne 15 -or $failed -ne 0 -or $notExecuted -ne 0) {
        throw "Ticket 09 gate requires exactly 15 passed and 0 skipped; observed total=$total executed=$executed passed=$passed failed=$failed notExecuted=$notExecuted."
    }

    Write-Output "MESINGEST_TICKET09_SQLSERVER_API_GATE_PASSED: passed=$passed skipped=$notExecuted expectedProductMajor=$ExpectedProductMajor expectedCompatibilityLevel=$ExpectedCompatibilityLevel trx=$trxPath"
}
finally {
